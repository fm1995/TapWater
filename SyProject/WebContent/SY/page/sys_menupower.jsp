...
<script type="text/javascript">
	var selectId = function() {
		...
		location.href = "/SyProject/sy/querySyMenupower?empId=" + id;
	}
...
<body>
	...
	<div id="wrapper">
		...
		<div id="content" class="xgrid">
			<form action="/SyProject/sy/queryEmpMenuPower" name="frm" method="post">
				<div class="x12">
					<h2>菜单权限</h2>
					...
					<div style="float: left;">
						<button class="btn-icon btn-star"
							onClick="location='/SyProject/SY/page/sys_menupower_set.jsp';">
							<span></span>批量设置菜单权限
						</button>
					</div>
						...
						<tbody>
							<c:forEach items="${syEmp}" var="m">
								<tr class="even gradeA">
									<td><input type="checkbox" id="ide" name="emp"
										value="${m.empId}" /></td>
									<td>${m.empId}</td>
									<td>${m.empNo}</td>
									<td>${m.empName}</td>
									...
									<td class="center"><input type="submit"
										class="btn-icon btn-small btn-blue btn-star" value="设置菜单权限" />
									</td>
								</tr>
							</c:forEach>
						</tbody>
