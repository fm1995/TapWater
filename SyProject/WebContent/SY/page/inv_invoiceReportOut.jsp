...
<link rel="stylesheet" href="/SyProject/SY/css/grid.css" type="text/css"
	media="screen" title="no title" />
<link rel="stylesheet" href="/SyProject/SY/css/layout.css" type="text/css"
	media="screen" title="no title" />

<link rel="stylesheet"
	href="/SyProject/SY/css/ui-darkness/jquery-ui-1.8.12.custom.css" type="text/css"
	media="screen" title="no title" />
<link rel="stylesheet" href="/SyProject/SY/css/plugin/jquery.visualize.css"
	type="text/css" media="screen" title="no title" />
<link rel="stylesheet" href="/SyProject/SY/css/plugin/facebox.css" type="text/css"
	media="screen" title="no title" />
<link rel="stylesheet" href="/SyProject/SY/css/plugin/uniform.default.css"
	type="text/css" media="screen" title="no title" />
<link rel="stylesheet" href="/SyProject/SY/css/plugin/dataTables.css"
	type="text/css" media="screen" title="no title" />

<link rel="stylesheet" href="/SyProject/SY/css/custom.css" type="text/css"
	media="screen" title="no title">
...
<script type="text/javascript">
		var selPage = function(fh)
		{
			...
			location.href="/SyProject/iv/outSelectAll?page="+page;
		}
</script>
</head>
	...
	<div id="wrapper">
		...
		<div id="header">
			...
			<div class="content_pad">
				<h1>
					<a href="/SyProject/SY/workspace.html">湖南省自来水公司营销管理信息系统</a>
				</h1>
		...
		<div id="masthead">
			<div>
				<span id="pagetitle"><a href="javascript:;">发票出库情况查询</a></span> <span
		...
		<div id="content" class="xgrid">
			...
			<div class="x12">
				...
				<div class="reportTitle">发票出库情况报表</div>
				<table class="report">
					<thead>
						<tr>
							<th width="60">出库序号</th>
					...
					<tbody>
					<c:forEach var="list" items="${data.outs}">
						<tr>
							<td class="center">${list.id}</td>
							<td class="center">${list.startNo}</td>
							<td class="center">${list.endNo}</td>
							<td class="right">${list.counts}</td>
							<td class="right">${list.usedCount}</td>
							<td class="right">${list.invalidCount}</td>
							<td class="right">${list.notUsedCount}</td>
							<td class="right">${list.archivedCount}</td>
							<td class="right">${list.notArchivedCount}</td>
							<td class="center">${list.outDate}</td>
							<td class="center">${list.outName}</td>
							<td class="center">${list.yongName}</td>
						</tr>
					</c:forEach>
					</tbody>
				</table>
				<div class="page">
					<a href="javascript:selPage(1)">第一页</a> <a href="javascript:selPage('-')">上一页</a> <input class="pageIndex"
						 id="page" value="${ data.pageClass.page}"/> / <input class="pageCount" readonly="readonly"
						id="pageCount" value="${data.pageClass.pageCount}"/> <a href="javascript:selPage('+')">下一页</a> <a href="javascript:selPage('*')">第未页</a>
				</div>
			</div>
			<!-- .x12 -->
